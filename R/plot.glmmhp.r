...
plot.glmmhp <- function(x, plot.perc = FALSE, color = NULL, n = 1, dig = 4, ...) {
  ...
  if (x$type == "hierarchical.partitioning") {
    if (plot.perc) {
      tips3 <- data.frame(
        variable = rownames(x[[n + 1]]),
        value = as.numeric(x[[n + 1]][, "I.perc(%)"])
      )
      gg <- ggplot2::ggplot(tips3, ggplot2::aes(x = stats::reorder(variable, -value), y = value)) +
        ggplot2::geom_bar(stat = "identity") +
        ggplot2::theme_minimal() +
        ggplot2::labs(x = "Variables", y = "% Individual effect to Rsquare (%I)") +
        ggplot2::theme(axis.text = element_text(size = 10)) +
        ggplot2::theme(axis.title = element_text(size = 13)) +
        ggplot2::labs(title = paste(names(x)[n + 1]))
    } else {
      tips2 <- data.frame(
        variable = rownames(x[[n + 1]]),
        value = as.numeric(x[[n + 1]][, "Individual"])
      )
      gg <- ggplot2::ggplot(tips2, ggplot2::aes(x = stats::reorder(variable, -value), y = value)) +
        ggplot2::geom_bar(stat = "identity") +
        ggplot2::theme_minimal() +
        ggplot2::labs(x = "Variables", y = "Individual effect") +
        ggplot2::theme(axis.text = element_text(size = 10)) +
        ggplot2::theme(axis.title = element_text(size = 13)) +
        ggplot2::labs(title = paste(names(x)[n + 1]))
    }
